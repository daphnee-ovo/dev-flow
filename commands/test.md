...
| task/ 目录下所有 task 文件内容（含 done_task_*） | git log / commit messages |
| 项目上下文（context.sh 输出） | 例行 TEST 的结果历史 |
| 当前日期（用于 issue 文件名） | |
...
项目上下文帮助 TEST agent 快速了解如何运行项目和已有测试结构，不需要花费大量 token 探索。

## 结果处理

- **全部通过** → 执行 /iterate 进行交付
