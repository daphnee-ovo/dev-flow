...
| `--files`     | Additional source code files/directories to commit (space-separated). **No need to pass .dev-doc/ files** — they're auto-managed by iterate (archive delete + `git add -u`). **Paths are validated before preIterate executes** — do not include files that will be dynamically generated by preIterate steps (those are auto-collected). | Optional |
...
- **patch** (default): The current VERSION is used as the released version (no bump). After release, next version = released + patch.
- **minor/major**: VERSION is first bumped to the target level, that becomes the released version. After release, next version = released + patch.

Example: VERSION is `1.2.3`
- `--bump patch` → releases `v1.2.3`, next version becomes `1.2.4`
- `--bump minor` → releases `v1.3.0`, next version becomes `1.3.1`
- `--bump major` → releases `v2.0.0`, next version becomes `2.0.1`
...
Before outputting the preview, iterate automatically triggers `save_changelog` to ensure current session activity is captured in CHANGELOG.md.

Outputs preview info: archive content, version number, tag to be created, commit file list, confirmation token.

If any persistent documents declared in STATUS.yaml `docs` (plus README.md) have not been updated since the last tag, a **doc sync warning** is displayed. Ensure these documents are up-to-date before confirming — iterate does not block on this, but outdated docs shipped with a release are a quality risk.
...
Before executing preIterate steps, iterate captures a full snapshot of all tracked and dirty files. If **any** step fails:
1. All file changes produced by earlier steps are rolled back to their pre-iterate state
2. The error is reported with a note that changes have been rolled back
3. No archive, no commit, no tag, no bump occurs

This ensures a failed preIterate never leaves the working tree in a half-modified state.
...
- After archive completes, `claim.lock` is automatically cleaned up (no longer needed for the next iteration)

## After Completion Output

```
[dev-flow] Iteration v2.2.0 Complete
```
